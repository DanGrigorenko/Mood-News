...
export type RewriteFeedback = {
  ...
  distortion: string
  // Текст прошлой попытки. Каждый вызов модели статeless (своего разговора у
  // ретрая нет, llm.ts шлёт один system+user без истории) — «сохрани уже
  // переписанный текст» пустые слова, если этого текста нет в самом запросе.
  // Ретраю есть что редактировать, только если он дословно видит, что писал.
...
export function buildMessages(brief: RewriteBrief): ChatMessage[] {
  ...
  const rules = [
    ...
    // «Не меняй имена» модель читала как «не подменяй лицо» и спокойно
    // переписывала их латиницей: «Мидзухо» → «Midzuhu», «Дойче Банк» → «Deyche
    // Bank». Сверка ищет написание источника, для неё это потеря Anchor. Правило
    // говорит только про азбуку: падеж имени безразличен и первой попытке
    // (docs/adr/0009), и сверке по первым пяти символам (anchor.ts).
    'Имена, фамилии и названия компаний, городов и стран пиши той же азбукой, что и в источнике: «Мидзухо» остаётся «Мидзухо», а не становится «Midzuhu». Не транслитерируй их латиницей, не переводи и не восстанавливай иностранное написание.',
  ...
  const howToRewrite = [
    ...
    // Оговорка «сокращай за счёт слов, а не за счёт цифр» здесь стояла и была
    // снята: замер после неё дал 5 Rewrite с потерянными числами против 3 до
    // (включая long/Грустно 3/14 — восемь предложений сжаты в три). Доказать
    // ухудшение одним шумным прогоном нельзя, но и оснований держать правило,
    // которое не помогло, а слово «сокращай» в промпте усилило, — тоже нет.
    ...
    // Длинный источник модель переписывает с начала, а дальше идёт за ним слово
    // в слово: замер — sim 0.66–0.86 и unchanged после трёх попыток. Правило
    // говорит про равномерность переписывания, а не про объём: сокращать
    // по-прежнему можно, нельзя перестать переписывать на середине.
    'Конец текста должен быть переписан не меньше, чем начало: чем длиннее источник, тем сильнее тянет переработать первые предложения, а дальше идти за ним слово в слово.',
  ...
  if (feedback) {
    // Вызов модели stateless — своей прошлой попытки она не помнит. Без этого
    // текста инструкции ниже («сохрани уже переписанное» или «перепиши
    // заново») не к чему прикладывать. Сам этот блок ничего не предписывает —
    // правь или переписывай с нуля решает ветка ниже (missing/unchanged), два
    // разных предписания в одном месте друг другу противоречили бы.
    userParts.push(
      'Твоя прошлая попытка:',
      `Заголовок: ${feedback.previous.title}`,
      `Текст: ${feedback.previous.body}`,
    )
  }
  ...
  if (feedback && feedback.unchanged) {
    ...
    if (feedback.surviving.length > 0) {
      userParts.push(
        // Пять провалов unchanged на замере пережили все три попытки: правило
        // про равномерность нужно и ретраю, а не только первой попытке, — иначе
        // ретрай переписывает начало ещё раз и снова бросает хвост.
        `Вот куски, которые ты перенёс из источника слово в слово, — перескажи именно их своими словами; обычно так уходит конец текста, а не начало: ${feedback.surviving.map((s) => `«${s}»`).join(', ')}.`,
      )
    }
  }
