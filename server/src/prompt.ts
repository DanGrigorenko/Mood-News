...
export function buildMessages(brief: RewriteBrief): ChatMessage[] {
  ...
  const howToRewrite = [
    ...
    // Разрешение опускать подробности модель распространяла и на цифры: замер на
    // живой модели, dramatic выбрасывал «60%» и «40%» из длинного текста.
    // Оговорка живёт в том же правиле, что и разрешение сокращать, — иначе
    // «сокращай» и «не теряй числа» стоят в разных пунктах и спорят друг с другом.
    'Не иди по источнику предложение за предложением — пересобери текст заново. Второстепенные подробности опустить можно, но сокращай за счёт слов, а не за счёт цифр: числа, имена, даты и суммы обязаны остаться все до единой.',
